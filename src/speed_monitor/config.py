...
class CalibrationConfig:
    ...
    This baseline implementation uses a meters-per-pixel scale, optionally
    interpolated by the detection's y-coordinate to roughly compensate for
    perspective.

    - If only `meters_per_pixel_near` is provided, that constant scale is used.
    - If `meters_per_pixel_far`, `y_near`, and `y_far` are also provided, the
      scale is linearly interpolated between (y_near, near) and (y_far, far).
    """

    fps: float = 30.0

    meters_per_pixel_near: float = 0.05

    meters_per_pixel_far: float | None = None
...
def _coerce_calibration(data: dict[str, Any]) -> CalibrationConfig:
    return CalibrationConfig(
        ...
        meters_per_pixel_near=float(data.get("meters_per_pixel_near", 0.05)),
        meters_per_pixel_far=(
            None
            if data.get("meters_per_pixel_far") is None
            else float(data["meters_per_pixel_far"])
        ),
...
def load_config(path: str | Path) -> MonitorConfig:
    ...
    return MonitorConfig(
        ...
        speed_limit_kmh=(
            None
            if payload.get("speed_limit_kmh") is None
            else float(payload["speed_limit_kmh"])
        ),
    )
